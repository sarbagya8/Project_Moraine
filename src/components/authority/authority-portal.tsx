    ...
    content = (
      <>
        <PageHeading
          ...
          description={`Last update: ${relativeAge(data.generatedAt)}. Emergency and device data refresh every 10 seconds while this page is visible.`}
    ...
    content = event ? (
      <>
        ...
        <section className="summary-grid">
          ...
          <DataCard label="Temperature" value={event.temperature == null ? "Unavailable" : `${event.temperature} °C`} />
    ...
    content = (
      <>
        ...
        <section className="panel prose-panel">
          ...
          <p>Use the deployment checklist and CREATOR guide to change these values. This page intentionally does not reveal their current contents.</p>
...
function TrekkerDetail({ trekker, history, attempts, freshness }: { trekker: PortalTrekker; history: PortalEmergency[]; attempts: AuthorityOverview["notificationAttempts"]; freshness: AuthorityOverview["freshness"] }) {
  ...
  return (
    <>
      ...
      <section className="summary-grid">
        <DataCard label="Heart rate" value={trekker.latestReading ? `${trekker.latestReading.heartRate} bpm` : "Unavailable"} detail={readingStale ? "Stale or unavailable" : relativeAge(trekker.latestReading?.capturedAt)} />
        <DataCard label="SpO₂" value={trekker.latestReading ? `${trekker.latestReading.spo2}%` : "Unavailable"} />
        <DataCard label="Temperature" value={trekker.latestReading?.temperature == null ? "Unavailable" : `${trekker.latestReading.temperature} °C`} />
      ...
      <section className="panel">
        ...
        {trekker.readingHistory.length ? (
          <div className="table-wrap"><table><thead><tr><th>Captured</th><th>Heart rate</th><th>SpO₂</th><th>Temperature</th><th>Altitude</th></tr></thead><tbody>
            {[...trekker.readingHistory].reverse().map((reading) => <tr key={`${reading.capturedAt}-${reading.heartRate}`}><td>{formatTime(reading.capturedAt)}</td><td>{reading.heartRate} bpm</td><td>{reading.spo2}%</td><td>{reading.temperature == null ? "Unavailable" : `${reading.temperature} °C`}</td><td>{reading.altitude == null ? "Unavailable" : `${reading.altitude} m`}</td></tr>)}
          </tbody></table></div>
